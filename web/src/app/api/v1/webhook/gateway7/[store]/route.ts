...
/**
 * POST /api/v1/webhook/Gateway7/[store]
 * Webhook unificado para o Gateway7 Checkout.
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ store: string }> }) {
  ...
  try {
    ...
    // Identifica o tipo de evento do Gateway7 Checkout:
    ...
    // Extração do identificador do pedido/transação/carrinho (com fallbacks para simulação do Gateway7)
    let orderId = String(
      payload.order_id ||
      payload.orderId ||
      payload.id ||
    ...
    // Se for modo de teste/simulação do Gateway7 e não veio ID explícito
    if (!orderId && (payload.test_mode || payload.is_test)) {
      orderId = "Gateway7_TEST_" + Date.now();
    }
    ...
    const isTestMode = Boolean(payload.test_mode || payload.is_test || orderId.startsWith("Gateway7_TEST_"));
    ...
    try {
      ...
      console.warn("[Gateway7 Webhook] Fallback de integração aplicado.");
    }
    ...
    // 3. Normalização dos dados do cliente e pedido do Gateway7 Checkout
    ...
    // Valor da transação (cobrindo todas as variações de payload do Gateway7 / Zedy / Gateways)
    let orderValue = 0;
    ...
    try {
      ...
      if (storeData?.telegram_bot_token && storeData?.telegram_chat_id && shouldNotify) {
        ...
        }).catch(err => console.error("[Telegram Push Error (Gateway7)]", err));
      }
    } catch (e) {
      console.error("Erro ao buscar store para Telegram (Gateway7):", e);
    }
    ...
    console.error("[Gateway7 Webhook Integration Error]:", error);
